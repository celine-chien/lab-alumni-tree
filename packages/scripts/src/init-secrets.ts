...
async function putIfMissing(suffix: string, value: string, type: 'String' | 'SecureString') {
  const name = `${site.ssmPrefix}/${suffix}`;
...
await putIfMissing('passphrase-question', '實驗室名稱縮寫（三個大寫英文字）：', 'String');
await putIfMissing('passphrase-answer', 'ABC', 'String');
await putIfMissing('token-secret', rand(40), 'SecureString');
await putIfMissing('admin-key', rand(32), 'SecureString');
